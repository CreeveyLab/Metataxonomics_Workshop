...
if (-s $clstr2) {
  ...
  $str = "$script_dir/cd-hit-otu-simulation.pl $ave_len $err $largest_chimaric $sd1";
}
else {
  ($i, $largest, $ave_len) = clstr_info1($clstr);
  print "Average read length $ave_len\n";
  print "Largest cluster from $clstr1 is $largest\n\n";
  $str = "$script_dir/cd-hit-otu-simulation.pl $ave_len $err $largest $sd1";
}
print "\n\nrunning command\n$str\n";
$cmd = `$str`;
print $cmd;
...
-f whether to perform chimera checking (true/false), default true
